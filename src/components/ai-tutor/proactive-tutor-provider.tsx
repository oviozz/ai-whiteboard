...
export default function ProactiveTutorProvider({
  ...
  // Use tldraw screenshot hook instead of html2canvas
  const { captureScreenshot, isReady: isScreenshotReady } = useTldrawScreenshot()
  ...
  const { activityState, idleTime, recordActivity } = useActivityMonitor({
  ...
  const scheduleAnalysis = useCallback(
    (trigger: "idle" | "stuck" | "periodic") => {
      if (!proactiveHintsEnabled || isAnalyzing || !isScreenshotReady) return
    ...
    [proactiveHintsEnabled, isAnalyzing, isScreenshotReady]
  )
  ...
  const runAnalysis = useCallback(
    async (trigger: string) => {
      ...
      try {
        // Take screenshot using tldraw's native export
        const screenshotBlob = await captureScreenshot()

        if (!screenshotBlob) {
          console.log("[ProactiveTutor] No screenshot available (empty canvas?)")
          setAnalyzing(false)
          return
        }
        ...
        if (result.success && result.analysis) {
          ...
          if (hintType === "quick" && briefHint) {
            // For quick hints, try to position near center of viewport
            if (editor) {
              const viewportBounds = editor.getViewportPageBounds()
              const worldX = viewportBounds.x + viewportBounds.w / 2
              const worldY = viewportBounds.y + viewportBounds.h / 3
              hint = {
                type: "quick",
                content: briefHint,
  ...
  return (
    <>
      ...
      {/* Canvas Quick Hint Overlay - Note: tldraw manages its own canvas */}
      <CanvasHint canvasRef={canvasRef} panOffset={panOffset} zoomLevel={zoomLevel} />
