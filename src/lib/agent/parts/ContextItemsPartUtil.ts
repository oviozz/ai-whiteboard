 ...
 *
 * Handles context items (shapes, areas, points) selected by the user.
 */
...
export class ContextItemsPartUtil extends PromptPartUtil<ContextItemsPart> {
  ...
  override getPart(
    ...
    const items = request.contextItems.map((contextItem) => {
      const offsetContextItem = helpers.applyOffsetToContextItem(
        JSON.parse(JSON.stringify(contextItem))
      );
